...
import {buildRanking, buildTeamsSuggestions, extractNicknameTags} from "./modules/rankProcessing/rankProcessing.ts"
...
export async function updateRankings(memberOld: GuildMember | PartialGuildMember, memberNew: GuildMember) {
    const channel = discord.channels().text().mix().matchmakingTeams
...
export function suggestTeams(oldState: VoiceState, newState: VoiceState) {
    ...
    const players = () => nicknames().map(extractNicknameTags)
